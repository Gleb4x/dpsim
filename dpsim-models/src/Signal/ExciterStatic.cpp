...
void ExciterStatic::initialize(Real Vh_init, Real Ef_init) {
	...
	SPDLOG_LOGGER_INFO(mSLog, 
		"Set excitation system initial values:"
		"\ninit Vh: {:e}"
		"\ninit Ef: {:e}",
		mVh, mEfd);
	...
	{
		SPDLOG_LOGGER_INFO(mSLog, 
			"\nThe initialisation is bad, Ef_init out of allowed band ... the simulation is continued ",
			this->name());
		throw CPS::TypeException();
	}
	...
	mVref = mVin+Vh_init;
	///initial value for auxilarz state variable
	mXb= mVin;
	mXb_next=mXb;
